...
function toggleBurger() {
  ...
  burgerLineBottom.style.y = expanded ? '45' : '0';
  burgerLineTop.style.y = expanded ? '45' : '80';
}

navToggle.addEventListener('click', toggleBurger);

navLink.forEach((navLink) => navLink.addEventListener('click', toggleBurger));

document.addEventListener('click', (event) => {
  const isInsideWrapper = event.target.closest('#wrapper');
  const visibility = navMenu.getAttribute('data-visible');
  if (isInsideWrapper && visibility === 'true') {
    toggleBurger();
  }
});
...
const container = document.querySelector('#prospects-set');
...
prospects.forEach((prospect) => {
  ...
  if (prospect.id > 2) prospectBody.classList.add('hidden', 'md:flex');

  prospectBody.innerHTML = `
